...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyTheme = (theme) => {
    console.log("Applying theme to settings window:", theme); // Debug log
    if (theme === "dark") {
...
).style.display = "flex"; // Show the default tab
...
document.getElementById("btn-save-setting").addEventListener("click", () => {
  config.panel.autoSave = document.getElementById(
    "switch-save-setting"
  ).checked;
  ...
  config.keyboard.customKeyboard = document.getElementById(
    "switch-custom-keyboard"
  ).checked;
  ...
  if (!config.update) {
    // Ensure the update object exists
    config.update = {};
  }
  config.update.blockUpdate = document.getElementById(
    "switch-block-update"
  ).checked;
